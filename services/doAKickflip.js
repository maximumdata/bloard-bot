...
const { createWriteStream, readdir, readFileSync, existsSync, fstat } = require('fs')
...
export default async function doAKickflip(message) {
    try {
        ...
        if (!existsSync(path.resolve(__dirname, '../kickflips', `${username}.gif`))) {
            let avi;
            ...
            const upload = await imgur(readFileSync(path.resolve(__dirname, '../kickflips', `${username}.gif`)) );
            await message.channel.send(upload.link);
        }
    } catch (error) {
        console.error(error);
